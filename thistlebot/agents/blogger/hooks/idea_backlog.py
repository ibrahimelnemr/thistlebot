...
def _build_user_prompt(
    *,
    ...
    lines = [
        ...
        "",
        "This workflow is laser-focused on latest AI news stories.",
        "Use DuckDuckGo News style queries when possible (for example: 'ai' on the News tab with newest-first).",
...
def _build_query_plan(*, topic: str, query_count: int, now: datetime) -> list[str]:
    ...
    seeds = [
        "ai",
        f"AI latest news {year}",
        f"AI breaking news last 24 hours {year}",
        f"AI major announcement today {month} {year}",
        f"OpenAI Anthropic Google Meta AI latest news {year}",
        f"AI regulation and policy latest news {year}",
        f"AI model release news {month} {year}",
        f"AI safety incident latest report {year}",
        f"AI funding acquisition news {year}",
        f"AI product launch news {month} {year}",
    ]
    if topic and topic.strip().lower() not in {"ai", "ai news", "latest ai news"}:
        seeds.insert(1, f"{topic} latest ai news {year}")
...
def _fallback_ideas(*, topic: str, count: int, now: datetime) -> list[dict[str, Any]]:
    ...
    ideas: list[dict[str, Any]] = []
    templates = [
        ("Breaking AI Story Tracker", "Fact-first report on the single biggest AI story right now"),
        ("Top AI Headline Today", "Neutral summary of the latest major AI announcement"),
        ("AI Policy Story Watch", "What the newest regulation headline means"),
        ("AI Company Announcement Brief", "Timeline and factual breakdown of the latest company update"),
        ("AI Model Launch Report", "What was announced, what is confirmed, and what is unknown"),
        ("AI Safety News Brief", "Objective update on the latest safety incident or claim"),
    ]
