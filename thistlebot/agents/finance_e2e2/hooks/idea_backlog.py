...
def refresh_idea_backlog(
    *,
    ...
    prompt = _IDEAS_PROMPT_PATH.read_text(encoding="utf-8")
    messages = [
...
class IdeaBacklogSelectHook:
    ...
    def execute(self, context: HookContext) -> HookResult:
        ...
        default_topic = str(context.agent_config.get("topic") or "Personal finance and making money")
...
def cli_refresh(
    ...
    **_: Any,
    ...
    return refresh_idea_backlog(
        ...
        registry=registry,
        model=model,
        topic=str(args.get("topic") or "Personal finance and making money"),
...
def _build_user_prompt(
    *,
    ...
    lines = [
        ...
        "",
        "This workflow is laser-focused on latest personal finance and money news stories.",
        "Use DuckDuckGo News style queries when possible (for example: 'personal finance' on the News tab with newest-first).",
...
def _build_query_plan(*, topic: str, query_count: int, now: datetime) -> list[str]:
    ...
    seeds = [
        "personal finance",
        f"personal finance latest news {year}",
        f"making money news last 24 hours {year}",
        f"interest rate and inflation update {month} {year}",
        f"credit card mortgage savings rate latest news {year}",
        f"retirement investing update latest news {year}",
        f"tax policy for households latest news {month} {year}",
        f"consumer debt trend latest report {year}",
        f"cost of living and wages latest news {year}",
        f"budgeting and spending trend news {month} {year}",
    ]
    if topic and topic.strip().lower() not in {"personal finance", "finance", "making money"}:
        seeds.insert(1, f"{topic} latest personal finance news {year}")
...
def _fallback_ideas(*, topic: str, count: int, now: datetime) -> list[dict[str, Any]]:
    ...
    ideas: list[dict[str, Any]] = []
    templates = [
        ("Breaking Personal Finance Story", "Fact-first report on the biggest money story right now"),
        ("Top Finance Headline Today", "Neutral summary of the latest household-finance headline"),
        ("Rates and Inflation Watch", "What the newest rates/inflation update means for consumers"),
        ("Household Budget Impact Brief", "Timeline and factual breakdown of immediate money impacts"),
        ("Investing and Retirement Update", "What changed, what is confirmed, and what remains uncertain"),
        ("Consumer Debt and Savings Brief", "Objective update on borrowing, saving, and spending trends"),
    ]
...
def _fetch_existing_posts_snapshot(*, limit: int = 80) -> list[dict[str, str]]:
    cfg = load_config()
    wp = cfg.get("wordpress", {}) if isinstance(cfg.get("wordpress"), dict) else {}
